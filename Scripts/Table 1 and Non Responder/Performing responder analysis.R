...
Intake_func_2 <- function(dataset){
  ...
  Intake <- eval(parse(text = paste(dataset, "$", 
    ...
    select(Respondent.ID, Patient.traject.ID, bmi, gewicht, lengte, alcohol, roken, eerderOK, 
           ...
           werkWeek, # Hoeveel uren werkt u op dit moment per week?
           ziekteDuur, # Hoeveel weken bent u tot nu toe ziekgemeld geweest als gevolg van de klacht waarvoor u behandeld wilt worden?
           letselschade) %>%
